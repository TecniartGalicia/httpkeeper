...
export class HttpRequestParser implements RequestParser {
    ...
    private parseRequestLine(line: string): { method: string, url: string } {
        ...
        if (match = /^(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|CONNECT|TRACE|LOCK|UNLOCK|PROPFIND|PROPPATCH|COPY|MOVE|MKCOL|MKCALENDAR|ACL|SEARCH|WEBSOCKET)\s+/i.exec(line)) {
            method = match[1];
