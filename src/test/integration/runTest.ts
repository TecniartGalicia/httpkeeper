...
const s=http.createServer((q,r)=>{
  let b='';q.on('data',c=>b+=c);q.on('end',()=>{
    ...
    if(u.startsWith('/estado/')){const c=Number(u.split('/')[2])||500;r.writeHead(c,{'content-type':'application/json','x-ruta':u});return r.end(JSON.stringify({error:'vaya',estado:c},null,1));}
    if(u==='/redirige'){r.writeHead(302,{location:'/destino'});return r.end();}
    if(u==='/lento'){return setTimeout(()=>{r.writeHead(200,{'content-type':'application/json','x-ruta':u});r.end(eco());},1500);}
    if(u==='/lista'){r.writeHead(200,{'content-type':'application/json','x-ruta':u});return r.end(JSON.stringify({items:[{id:7},{id:9}]}));}
    if(u==='/json'){r.writeHead(200,{'content-type':'application/json','x-ruta':u});return r.end(JSON.stringify({anidado:{a:1,b:[1,2,3]}}));}
    ...
    r.writeHead(200,{'content-type':'application/json','x-ruta':u});r.end(eco());
  });
});
