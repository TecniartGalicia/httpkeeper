# HttpKeeper — reference documentation

This is the complete reference of the `.http` format and every feature inherited from REST Client, kept **verbatim** from Huachao Mao's original README (MIT) so that nothing anyone relied on is lost. Two things to read it with:

- Where it says a `rest-client.*` setting, HttpKeeper reads it **and** the equivalent `httpkeeper.*` setting (yours win when set). Commands live under the **HttpKeeper** category in the Command Palette, with the same names.
- Everything below still works exactly as described. What HttpKeeper **adds** on top (JetBrains environment files, `import` / `run #name`, `{{$secret}}`, `# @assert`, running a whole file, `text/event-stream` and `WEBSOCKET`, the terminal runner, tools for agents) is documented in the [README](../README.md) and the [CHANGELOG](../CHANGELOG.md). Nothing was removed.
